#!/usr/bin/env python
"""Republishes /hesai/pandar in FAST-LIO2's expected velodyne point layout.

Runs inside the fast_lio2 container (needs rospy); the conversion itself is the pure,
unit-tested `to_fast_lio2_points` in pointcloud_adapter.py.
"""
import numpy as np
import rospy
from pointcloud_adapter import OUT_DTYPE, to_fast_lio2_points
...
def main():
    rospy.init_node("fast_lio2_pointcloud_adapter")
    in_topic = rospy.get_param("~in_topic", "/hesai/pandar")
    out_topic = rospy.get_param("~out_topic", "/hesai/pandar/fast_lio2")
