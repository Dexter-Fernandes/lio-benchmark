...
`log_to_wandb` mirrors a finished record to Weights & Biases; W&B tracking is central to this
project, so WANDB_MODE defaults to online (set WANDB_MODE=offline in .env to develop without
W&B credentials, then `wandb sync runs/<run_id>/wandb/offline-run-*` later).
"""
...
def log_to_wandb(run_dir: Path) -> str:
    """Mirror a run record to W&B (online unless WANDB_MODE=offline). Returns the W&B run path."""
    import wandb
    ...
    wb = wandb.init(
        ...
        mode=setting("WANDB_MODE", "online"), dir=str(run_dir), name=run["run_id"], id=run["run_id"],
...
REPORT_METRICS = ["ate.trans_m.rmse", "ate.trans_m.p95", "ate.rot_deg.rmse",
                  "rpe.1s.trans_m.rmse", "rpe.10s.trans_m.rmse", "coverage.coverage"]


def build_report(project: str | None = None, entity: str | None = None) -> str:
    """Create or update the one saved W&B Report comparing every run in the project.

    Bar panels, one per key metric, each showing every logged run (not just local ones), so
    the report stays current as more methods/sequences are logged from any machine.
    """
    import wandb
    import wandb_workspaces.reports.v2 as wr

    project = project or setting("WANDB_PROJECT", "lio-benchmark")
    entity = entity or setting("WANDB_ENTITY") or wandb.Api().default_entity

    existing = [r for r in wandb.Api().reports(f"{entity}/{project}") if r.display_name == REPORT_TITLE]
    report = wr.Report.from_url(existing[0].url) if existing else wr.Report(
        project=project, entity=entity, title=REPORT_TITLE,
        description="Auto-generated by `lio-bench report`. Re-run it to refresh after logging new runs.")

    report.blocks = [wr.PanelGrid(
        runsets=[wr.Runset(entity=entity, project=project)],
        panels=[wr.BarPlot(title=metric, metrics=[metric]) for metric in REPORT_METRICS],
    )]
    report.save()
    return report.url
